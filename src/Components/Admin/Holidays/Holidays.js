...
const Holidays = () => {
  ...
  useEffect(() => {
    fetch("https://hr-care-nurulislameees-projects.vercel.app/holidays")
      .then((res) => res.json())
  ...
  useEffect(() => {
    findDate.map((onedate) =>
      fetch(
        `https://hr-care-nurulislameees-projects.vercel.app/attendance/${onedate._id}`,
        {
          method: "DELETE",
        }
      )
  ...
  const onSubmit = (data, e) => {
    ...
    for (
      ...
      employees.map((user) =>
        fetch("https://hr-care-nurulislameees-projects.vercel.app/attendance", {
          method: "POST",
          headers: { "content-type": "application/json" },
          body: JSON.stringify({
            ID: user?.ID,
  ...
  const handleDelete = (item) => {
    fetch(
      `https://hr-care-nurulislameees-projects.vercel.app/holidays/${item._id}`,
      {
        method: "DELETE",
      }
    )
    ...
    for (
      ...
      fetch(
        `https://hr-care-nurulislameees-projects.vercel.app/attendance/date/${currentDate}`
      )
        .then((res) => res.json())
  ...
  return (
    <Container>
        ...
        <Box>
          ...
          <Breadcrumbs aria-label="breadcrumb">
            ...
            <Link to="/dashboard/holiday">
              <StyledBreadcrumb component="a" href="#" label="Holiday" />
            </Link>
          </Breadcrumbs>
        </Box>
      ...
      >
        <Table
          sx={{ width: "100%", overflowX: "scroll", whiteSpace: "nowrap" }}
          ...
          <TableBody>
            {holidays.map((item) => (
              <StyledTableRow>
                ...
                <StyledTableCell align="center">{item.endDate}</StyledTableCell>
